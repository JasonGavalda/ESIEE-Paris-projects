...
public class VisiteurInstToString extends VisiteurInstruction<String>{
   ...
   */
  public VisiteurInstToString(VisiteurExpression<String> vi, VisiteurExpressionBooleenne<String> vb){
    this.vi = vi;
    this.vb = vb;
  }
  ...
  public String visite(Selection sel){
    ...
    if(sel.i2() != null) 
      str = str + " sinon " + sel.i2().accepter(this);
   ...
   public String visite(Pour pour){
     return "pour(" + pour.init().accepter(this) + "," + pour.cond().accepter(this.vb) + "," + pour.inc().accepter(this) + ") " +
             pour.i1().accepter(this) + " fpour";
   }
