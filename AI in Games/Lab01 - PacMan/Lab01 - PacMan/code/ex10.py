...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if "exit" in self.getLegalActions(state) or len(self.getLegalActions(state)) == 0 : 
          return "exit"
        best_actions = []
        for action in self.getLegalActions(state):
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.q_table[state][action] = (1-self.alpha)*self.getQValue(state, action)+self.alpha*(reward+self.discount*self.computeValueFromQValues(next_state))
